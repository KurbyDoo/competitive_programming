...
int main() {
    ...
    for (int i = 1; i <= h; i++) {
        ...
        dp[i] = vector<ll>(w + 1, 999999999999999LL);
        for (int j = 1; j <= w; j++) {
            cin >> rows[i][j];
    ...
    dp[h][w] = max(0LL, pk[h + w - 1] - rows[h][w]);
    for (int i = h; i > 0; i--) {
        for (int j = w; j > 0; j--) {
            if (i < h) dp[i][j] = min(dp[i][j], max(0LL, dp[i + 1][j] + pk[i + j - 1] - rows[i][j]));
            if (j < w) dp[i][j] = min(dp[i][j], max(0LL, dp[i][j + 1] + pk[i + j - 1] - rows[i][j]));
        }
    }
    printf("%lld\n", dp[1][1]);
