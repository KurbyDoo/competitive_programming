...
int findRadius(int node)
{
    ...
    {
        if (max(prevDist[curN], curD - prevDist[curN]) < radius)
        {
            radius = max(prevDist[curN], curD - prevDist[curN]);
            centerNode = curN
        }
...
int travelTime(int N, int M, int L, int A[], int B[], int T[])
{
    ...
    if (N == 1)
        return 0;
    if (maxDiameter == 0 && N > 2)
        return L * 2;
    if (M == N - 2)
        return max(maxDiameter, maxRadius1 + maxRadius2 + L);
